...
Go Viam module for converting a pointcloud into a spatialmath.mesh

notes on what I want this module to be capable of

1. we take in a pointcloud
2. we do outlier removal on the pointcloud
3. (for not optional) we achieve uniform density of points
4. we return that as a pointcloud
5. we turn the pointcloud into a mesh
6. we return the mesh

PUT THIS IN THE RDK YOU ARE RUNNING
THE CONTENTS OF THIS .TXT FILE SHOULD REPLACE WHAT IS IN MESH.GO WITHIN THE SPATIALMATH PACKAGE IN RDK
...
// The given point _MUST_ be coplanar with the triangle. If it is known ahead of time that the point is coplanar, this is faster.
...
// must be <= 90 degrees.
func (t _Triangle) ClosestInsidePoint(point r3.Vector) (r3.Vector, bool) {
// Parametrize the triangle s.t. a point inside the triangle is
// Q = p0 + u _ e0 + v * e1, when 0 <= u <= 1, 0 <= v <= 1, and
